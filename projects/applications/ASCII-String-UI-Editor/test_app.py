...
def test_presets():
    ...
    for name, charset in PRESETS.items():
        for key in required_keys:
            ...
            assert isinstance(charset[key], str), f"Preset {name} key {key} must be string"
            assert len(charset[key]) > 0, f"Preset {name} key {key} cannot be empty"
...
def test_renderers():
    ...
    for name, charset in PRESETS.items():
        ...
        assert len(patterns) == 5, f"Expected 5 patterns for {name}, got {len(patterns)}"

        for pattern in patterns:
            assert 'title' in pattern, "Pattern missing title"
            assert 'content' in pattern, "Pattern missing content"
            assert isinstance(pattern['content'], str), "Pattern content must be string"
            assert len(pattern['content']) > 0, "Pattern content cannot be empty"
...
def test_unicode_characters():
    ...
    unicode_presets = ['SINGLE_LINE', 'DOUBLE_LINE', 'HEAVY_LINE', 'ROUNDED_LINE']
