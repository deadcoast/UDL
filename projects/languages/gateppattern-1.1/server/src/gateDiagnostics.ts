...
export function getDiagnostics(doc: TextDocument): Diagnostic[] {
  ...
  if (breakIdx !== -1 && !text.match(/\?\s*confirm:\s*"YES"/)) {
    ...
    diagnostics.push({
      message:
        'break_gate detected without explicit ? confirm:"YES" ceremony confirmation.',
  ...
  if (text.includes("!!IRREVERSIBLE") && text.includes('STATUS="ACTIVE"')) {
    diagnostics.push({
      message:
        'Irreversibility marker (!!IRREVERSIBLE) present while a gate is still STATUS="ACTIVE". Check gate state sequencing.',
